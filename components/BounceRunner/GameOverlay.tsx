    ...
    return (
      <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-start pointer-events-none select-none z-20">
        <div className="flex flex-col">
          ...
          <span className="text-4xl font-bold text-white font-mono drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]">
        ...
        <div className="flex flex-col items-end opacity-70">
          ...
          <span className="text-xl font-bold text-white font-mono">{Math.floor(highScore)}m</span>
        </div>
      </div>
    );
  }
    ...
    <div className="absolute bottom-12 left-1/2 transform -translate-x-1/2 w-full max-w-lg px-4 animate-fade-in-up">
      <div className="bg-[#0f0f17]/90 border border-white/10 backdrop-blur-md p-4 rounded-xl shadow-2xl">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-white text-sm font-bold uppercase tracking-widest">Select Visual Core</h3>
          ...
          {THEMES.map(theme => {
            ...
            return (
              ...
              >
                ...
                />
                <span className="text-[10px] text-gray-300 uppercase font-bold">{theme.name}</span>
                {!isUnlocked && (
                  <div className="absolute inset-0 flex items-center justify-center bg-black/60 rounded-lg backdrop-blur-[1px]">
                    <span className="text-[9px] text-white font-mono bg-black/50 px-1 rounded border border-white/20">
                      {theme.unlockScore}m
                    </span>
                  </div>
                )}
    ...
    return (
      <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0f0f17]/80 backdrop-blur-sm z-10">
        <div className="mb-12 text-center animate-fade-in-up">
          <h1 className="text-6xl md:text-8xl font-black text-white italic tracking-tighter mb-2 drop-shadow-[0_0_25px_rgba(115,61,242,0.6)]">
            BOUNCE
  ...
  if (status === GameStatus.GAME_OVER) {
    ...
    return (
      <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0f0f17]/90 backdrop-blur-md z-10 animate-fade-in">
        <div className="mb-2 text-[#ff4444] font-bold tracking-widest uppercase text-sm animate-pulse">Connection Lost</div>
        <h2 className="text-5xl md:text-7xl font-bold text-white mb-6 tracking-tighter">TERMINATED</h2>
        ...
        <div className="flex gap-12 mb-10 text-center relative">
          <div className="relative group">
            ...
            <div className="text-5xl font-mono text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.3)]">{Math.floor(score)}m</div>
          </div>
          <div>
            <div className="text-gray-500 text-xs uppercase mb-1">Record</div>
            <div className="text-5xl font-mono text-[#733DF2]">{Math.floor(highScore)}m</div>
          </div>
          
          {nextUnlock && (
            <div className="absolute top-[-60px] left-1/2 transform -translate-x-1/2 w-max bg-gradient-to-r from-yellow-500 to-orange-500 text-black font-bold px-4 py-1 rounded-full text-xs shadow-lg animate-bounce">
